...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/core/Permissions/permissionsSlice";
import type { RootState, AppDispatch } from "../../../../../redux/store";



const LS_KEY = "permissions_table_columns";
...
const PermissionsIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.permissionsUI
  );

  const { data, isLoading } = useFetchPermissionsQuery(filters);
  const [deletePermissions] = useDeletePermissionsMutation();
  const [importPermissions] = useImportPermissionsMutation();
  const [exportPermissions] = useExportPermissionsMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importPermissions(formData);
  ...
  const handleExport = async () => {
    try {
      await exportPermissions({
        uuids: data?.data.data.map((permission) => permission.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export permissions"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Permissions"
        subtitle="Your Permissions Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Permissions", active: true },
        ]}
        url="/core/permissions/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Permissions List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() => navigate(`/core/permissions/edit/${row.uuid}`)}
                      >
                        Edit
                      </button>
                      <button
