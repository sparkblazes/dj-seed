...
//   { key: "meta_title", label: "Meta_Title" },
//   { key: "description", label: "Description" },
//   { key: "order", label: "Order" },
//   { key: "status", label: "status" },
// ];

// const DEFAULT_COLUMNS = data?.visible_columns;
const LS_KEY = "faqCategories_table_columns";

const FaqCategoriesIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.faqCategoriesUI
  );

  const { data, isLoading } = useFetchFaqCategoriesQuery(filters);
  const [deleteFaqCategories] = useDeleteFaqCategoriesMutation();
  const [importFaqCategories] = useImportFaqCategoriesMutation();
  const [exportFaqCategories] = useExportFaqCategoriesMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importFaqCategories(formData);
  ...
  const handleExport = async () => {
    try {
      await exportFaqCategories({
        uuids: data?.data.data.map((faqCategories: any) => faqCategories.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export FaqCategories"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="FaqCategories"
        subtitle="Your FaqCategories Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "FaqCategories", active: true },
        ]}
        url="/engagement/faqCategories/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">FaqCategories List</h4>
              ...
              {isLoading ? (
                ...
                <DataTable
                  data={data?.data?.data || []}
                  columns={data?.columns || []}        // string[] is fine
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() => navigate(`/engagement/faqCategories/edit/${row.uuid}`)}
                      >
                        Edit
                      </button>
                      <button
...
export default FaqCategoriesIndex;
