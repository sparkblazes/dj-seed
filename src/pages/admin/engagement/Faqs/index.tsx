...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../redux/engagement/Faqs/faqSlice";
...
const LS_KEY = "faqs_table_columns";
...
const FaqsIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.faqUI
  );

  const { data, isLoading } = useFetchFaqsQuery(filters);
  const [deleteFaq] = useDeleteFaqMutation();
  const [importFaqs] = useImportFaqsMutation();
  const [exportFaqs] = useExportFaqsMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importFaqs(formData);
  ...
  const handleExport = async () => {
    try {
      await exportFaqs({
        uuids: data?.data.data.map((faq: any) => faq.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export faqs"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Faqs"
        subtitle="Your Faqs Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Faq", active: true },
        ]}
        url="/engagement/faqs/create"
        urlIcon="plus"
      />
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/engagement/faqs/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
