...
const LS_KEY = "jobs_table_columns";
...
const JobsIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.jobUI
  );

  const { data, isLoading } = useFetchJobsQuery(filters);
  const [deleteJob] = useDeleteJobMutation();
  const [importJobs] = useImportJobsMutation();
  const [exportJobs] = useExportJobsMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importJobs(formData);
  ...
  const handleExport = async () => {
    try {
      await exportJobs({
        uuids: data?.data.data.map((job) => job.uuid.toString()) || [],
      });
  ...
  return (
    <>
      <Breadcrumb
        title="Jobs"
        subtitle="Your Jobs Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Jobs", active: true },
        ]}
        url="/engagement/jobs/create"
        urlIcon="plus"
      />
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/engagement/jobs/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
