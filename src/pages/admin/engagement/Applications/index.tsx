...
const LS_KEY = "applications_table_columns";
...
const ApplicationIndex: React.FC = () => {
  ...
  const { data, isLoading } = useFetchApplicationsQuery(filters);
  const [deleteApplications] = useDeleteApplicationMutation();
  const [importApplications] = useImportApplicationsMutation();
  const [exportApplications] = useExportApplicationsMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importApplications(formData);
  ...
  const handleExport = async () => {
    try {
      await exportApplications({
        uuids: data?.data.data.map((applications: any) => applications.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export applications"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Applications"
        subtitle="Your Applications Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Application", active: true },
        ]}
        url="/engagement/applications/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Apllications List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/engagement/applications/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
