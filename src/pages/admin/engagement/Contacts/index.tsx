...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/engagement/Contacts/contactsSlice";
...
// const DEFAULT_COLUMNS = ["id", "name", "email", "phone", "subject"];
const LS_KEY = "contacts_table_columns";
...
const ContactsIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.contactsUI
  );

  const { data, isLoading } = useFetchContactsQuery(filters);
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importContacts(formData);
  ...
  const handleExport = async () => {
    try {
      await exportContacts({
        uuids: data?.data.data.map((contacts: any) => contacts.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export contacts"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Contacts"
        subtitle="Your Contacts Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Contacts", active: true },
        ]}
        url="/engagement/contacts/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Contacts List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/engagement/contacts/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
