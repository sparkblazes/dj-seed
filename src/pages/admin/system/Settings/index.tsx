...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/system/Settings/settingSlice";
...
const LS_KEY = "settings_table_columns";
...
const SettingsIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.settingUI
  );

  const { data, isLoading } = useFetchSettingsQuery(filters);
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importSettings(formData);
  ...
  const handleExport = async () => {
    try {
      await exportSettings({
        uuids: data?.data.data.map((setting: any) => setting.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export settings"] }],
        },
      ]);
    }
  };
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/system/settings/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
