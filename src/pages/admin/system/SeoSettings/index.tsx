...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/system/SeoSettings/seoSettingSlice";
...
const LS_KEY = "seoSettings_table_columns";

const SeoSettingsIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.seoSettingUI
  );

  const { data, isLoading } = useFetchSeoSettingsQuery(filters);
  const [deleteSeoSetting] = useDeleteSeoSettingMutation();
  const [importSeoSettings] = useImportSeoSettingsMutation();
  const [exportSeoSettings] = useExportSeoSettingsMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this seoSettings?")) {
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importSeoSettings(formData);
  ...
  const handleExport = async () => {
    try {
      await exportSeoSettings({
        uuids: data?.data.data.map((seoSettings: any) => seoSettings.id.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export seoSettings"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="SeoSettings"
        subtitle="Your SeoSettings Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "SeoSettings", active: true },
        ]}
        url="/system/seosettings/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Downloads List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/system/seosettings/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
