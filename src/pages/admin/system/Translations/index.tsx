...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/system/Translations/translationSlice";
...
const LS_KEY = "translations_table_columns";

const TranslationsIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.translationUI
  );

  const { data, isLoading } = useFetchTranslationsQuery(filters);
  const [deleteTranslation] = useDeleteTranslationMutation();
  const [importTranslations] = useImportTranslationsMutation();
  const [exportTranslations] = useExportTranslationsMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this translations?")) {
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importTranslations(formData);
  ...
  const handleExport = async () => {
    try {
      await exportTranslations({
        uuids: data?.data.data.map((translations: any) => translations.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export translations"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Translation"
        subtitle="Your Translation Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Translation", active: true },
        ]}
        url="/system/translations/create"
        urlIcon="plus"
      />
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/system/translations/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
