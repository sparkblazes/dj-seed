...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/system/Downloads/downloadSlice";
...
const LS_KEY = "downloads_table_columns";

const DownloadsIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.downloadUI
  );

  const { data, isLoading } = useFetchDownloadsQuery(filters);
  const [deleteDownload] = useDeleteDownloadMutation();
  const [importDownloads] = useImportDownloadsMutation();
  const [exportDownloads] = useExportDownloadsMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importDownloads(formData);
  ...
  const handleExport = async () => {
    try {
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export downloads"] }],
        },
      ]);
    }
  };
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/system/downloads/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
