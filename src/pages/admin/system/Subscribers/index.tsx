...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/system/Subscribers/subscriberSlice";
...
const LS_KEY = "subscribers_table_columns";

const SubscribersIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.subscriberUI
  );

  const { data, isLoading } = useFetchSubscribersQuery(filters);
  const [deleteSubscriber] = useDeleteSubscriberMutation();
  const [importSubscribers] = useImportSubscribersMutation();
  const [exportSubscribers] = useExportSubscribersMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this subscribers?")) {
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importSubscribers(formData);
  ...
  const handleExport = async () => {
    try {
      await exportSubscribers({
        uuids: data?.data.data.map((subscribers: any) => subscribers.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export subscribers"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Subscribers"
        subtitle="Your Downloads Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Subscribers", active: true },
        ]}
        url="/system/subscribers/create"
        urlIcon="plus"
      />
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/system/subscribers/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
