...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/BlogCategories/blogCategorieSlice";
...
const LS_KEY = "blogCategories_table_columns";

const BlogCategoriesIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.blogcategorieUI
  );

  const { data, isLoading } = useFetchBlogCategoriesQuery(filters);
  const [deleteBlogCategorie] = useDeleteBlogCategorieMutation();
  const [importBlogCategories] = useImportBlogCategoriesMutation();
  const [exportBlogCategories] = useExportBlogCategoriesMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this blogCategorie?")) {
      await deleteBlogCategorie(id);
    }
  };
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importBlogCategories(formData);
  ...
  const handleExport = async () => {
    try {
      await exportBlogCategories({
        uuids: data?.data.data.map((blogCategorie) => blogCategorie.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export blogCategories"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        ...
        subtitle="Your Blog Categories Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "BlogCategories", active: true },
        ]}
        url="/cms/blog-categories/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">BlogCategories List</h4>
              ...
              <div className="row mb-3">
                ...
                <div className="col-md-3">
                  <input
                    ...
                    placeholder="Search blogCategories..."
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/blog-categories/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
