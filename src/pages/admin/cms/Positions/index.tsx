...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/Positions/positionSlice";
...
//   { key: "background_image", label: "Background_image" },
//   { key: "video_url", label: "Video_url" },
//   { key: "button_text", label: "Button_text" },
//   { key: "status", label: "Status" },
// ];

// const DEFAULT_COLUMNS = ["id", "title", "department", "location", "status"];
const LS_KEY = "positions_table_columns";

const PositionsIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.positionsUI
  );

  const { data, isLoading } = useFetchPositionsQuery(filters);
  const [deletePositions] = useDeletePositionMutation();
  const [importPositions] = useImportPositionsMutation();
  const [exportPositions] = useExportPositionsMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importPositions(formData);
  ...
  const handleExport = async () => {
    try {
      await exportPositions({
        uuids: data?.data.data.map((position) => position.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export positions"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Positions"
        subtitle="Your Positions Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Positions", active: true },
        ]}
        url="/cms/positions/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Positions List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/positions/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
