...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/Services/serviceSlice";
...
const LS_KEY = "services_table_columns";
...
const ServicesIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.serviceUI
  );

  const { data, isLoading } = useFetchServicesQuery(filters);
  const [deleteService] = useDeleteServiceMutation();
  const [importServices] = useImportServicesMutation();
  const [exportServices] = useExportServicesMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importServices(formData);
  ...
  const handleExport = async () => {
    try {
      await exportServices({
        uuids: data?.data.data.map((service) => service.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export services"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Services"
        subtitle="Your Services Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Services", active: true },
        ]}
        url="/cms/services/create"
        urlIcon="plus"
      />
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/services/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
