...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/PortfolioCategories/portfolioCategorieSlice";
...
const LS_KEY = "portfolioCategories_table_columns";

const PortfolioCategoriesIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.portfolioCategorieUI
  );

  const { data, isLoading } = useFetchPortfolioCategoriesQuery(filters);
  const [deletePortfolioCategorie] = useDeletePortfolioCategorieMutation();
  const [importPortfolioCategories] = useImportPortfolioCategoriesMutation();
  const [exportPortfolioCategories] = useExportPortfolioCategoriesMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this portfolioCategorie?")) {
      await deletePortfolioCategorie(id);
    }
  };
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importPortfolioCategories(formData);
  ...
  const handleExport = async () => {
    try {
      await exportPortfolioCategories({
        uuids: data?.data.data.map((portfolioCategorie) => portfolioCategorie.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export portfolioCategories"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        ...
        subtitle="Your Portfolio Categories Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "PortfolioCategories", active: true },
        ]}
        url="/cms/portfolio-categories/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">PortfolioCategories List</h4>
              ...
              <div className="row mb-3">
                ...
                <div className="col-md-3">
                  <input
                    ...
                    placeholder="Search portfolioCategories..."
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/portfolio-categories/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
...
export default PortfolioCategoriesIndex;
