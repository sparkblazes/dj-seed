...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/Galleries/gallerieSlice";
...
const LS_KEY = "galleries_table_columns";

const GalleriesIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.gallerieUI
  );

  const { data, isLoading } = useFetchGalleriesQuery(filters);
  const [deleteGallerie] = useDeleteGallerieMutation();
  const [importGalleries] = useImportGalleriesMutation();
  const [exportGalleries] = useExportGalleriesMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this gallerie?")) {
      await deleteGallerie(id);
    }
  };
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importGalleries(formData);
  ...
  const handleExport = async () => {
    try {
      await exportGalleries({
        uuids: data?.data.data.map((gallerie: any) => gallerie.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export galleries"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Galleries"
        subtitle="Your Galleries Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Galleries", active: true },
        ]}
        url="/cms/galleries/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Galleries List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/galleries/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
