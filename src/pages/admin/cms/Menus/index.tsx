...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/Menus/menuSlice";
...
const LS_KEY = "menus_table_columns";

const MenusIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.menuUI
  );

  const { data, isLoading } = useFetchMenusQuery(filters);
  const [deleteMenu] = useDeleteMenuMutation();
  const [importMenus] = useImportMenusMutation();
  const [exportMenus] = useExportMenusMutation();
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importMenus(formData);
  ...
  const handleExport = async () => {
    try {
      await exportMenus({
        uuids: data?.data.data.map((menu) => menu.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export menus"] }],
        },
      ]);
    }
  };
  return (
    <>
      <Breadcrumb
        title="Menus"
        subtitle="Your Menus Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Menus", active: true },
        ]}
        url="/cms/menus/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">Menus List</h4>
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/menus/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
