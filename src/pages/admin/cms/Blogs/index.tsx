...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/cms/Blogs/blogSlice";
...
const LS_KEY = "blogs_table_columns";
...
const BlogsIndex: React.FC = () => {
  ...
  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.blogUI
  );

  const { data, isLoading } = useFetchBlogsQuery(filters);
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importBlogs(formData);
  ...
  const handleExport = async () => {
    try {
      await exportBlogs({
        uuids: data?.data.data.map((blog: any) => blog.uuid.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export blogs"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        title="Blogs"
        subtitle="Your Blogs Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "Blogs", active: true },
        ]}
        url="/cms/blogs/create"
        urlIcon="plus"
      />
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/cms/blogs/edit/${row.uuid}`)
                        }
                      >
                        Edit
                      </button>
