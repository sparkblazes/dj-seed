...
import {
  setFilters,
  setVisibleColumns,
} from "../../../../../redux/ecommerce/ProductCategories/productCategorieSlice";
...
const LS_KEY = "productCategories_table_columns";

const ProductCategoriesIndex: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();

  const { filters, visibleColumns } = useSelector(
    (state: RootState) => state.productCategorieUI
  );

  const { data, isLoading } = useFetchProductCategoriesQuery(filters);
  const [deleteProductCategorie] = useDeleteProductCategorieMutation();
  const [importProductCategories] = useImportProductCategoriesMutation();
  const [exportProductCategories] = useExportProductCategoriesMutation();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this productCategorie?")) {
      await deleteProductCategorie(id);
    }
  };
  ...
  const handleImport = async (file: File) => {
    ...
    const res: any = await importProductCategories(formData);
  ...
  const handleExport = async () => {
    try {
      await exportProductCategories({
        uuids: data?.data.map((productCategorie) => productCategorie.id.toString()) || [],
      });
      ...
      setImportErrors([
        {
          row: "-",
          field: [{ attribute: "export", errors: ["Failed to export productCategories"] }],
        },
      ]);
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        ...
        subtitle="Your ProductCategories Module."
        items={[
          { label: "Dashboard", href: "/admin" },
          { label: "ProductCategories", active: true },
        ]}
        url="/ecommerce/product-categories/create"
        urlIcon="plus"
      />
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="d-flex justify-content-between align-items-center mb-3">
                {/* Title */}
                <h4 className="card-title mb-0">ProductCategories List</h4>
              ...
              <div className="row mb-3">
                ...
                <div className="col-md-3">
                  <input
                    ...
                    placeholder="Search productCategories..."
                  ...
                  actions={(row) => (
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(`/ecommerce/productCategories/edit/${row.id}`)
                        }
                      >
                        Edit
                      </button>
